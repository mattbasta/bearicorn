/*
Requires the following leaderboards:
    rock_jumps, hit_by_rock, molsons, jumps, fell_in_hole, stages, died,
    games, et, secret_moose
*/
...
navigator.game.configure('bearicorn');
navigator.game.authenticate();
navigator.game.playing();
...
var states = [
    ...
    function(ratio) {
        ...
        for (var i = 0, r; r = rocks[i++];) {
            ...
            if (abs(r[1] - y - 155) < 15 &&
                ...
                if (vy < 0) {
                    vy = min(vy + 12, 7);
                    rock_reset();
                    navigator.game.updateScore('rock_jumps', 1);
                } else {
                    navigator.game.updateScore('hit_by_rock', 1);
        ...
        if (beerpos + 1) {
            ...
            if (abs(x + 5 - bx) < 10 && y > -10) {
                navigator.game.updateScore('molsons', 1);
                difficulty /= 2;
                beerpos = -1;
                mticks = 100;
                message = 'MOLSON!';
        ...
        if (keys.jump) {
            if (y === floor) {
                navigator.game.updateScore('jumps', 1);
            }
        ...
        if (y > 50) {
            navigator.game.updateScore('fell_in_hole', 1);
        ...
        if (x > 290) {  // Made it to the end of the stage
            navigator.game.updateScore('stages', 1);
    ...
    function(ratio) {
        ...
        if(death_count >= 60) {
            navigator.game.updateScore('died', 1);
            setups[state = 6]();
        }
    },
    // 6 - Lost
...
var setups = [
    ...
    // 1 - Level 1
    function(soft) {
        navigator.game.updateScore('games', 1);
        mticks = 0;
        if (!soft) {
            difficulty = 0;
            beerpos = -1;
    ...
    function() {
        ...
        if (random() * 200 | 0) {
        // if (0) {
            navigator.game.updateScore('et', 1);
            drawET(165, 145);
        } else {
            navigator.game.updateScore('secret_moose', 1);
            ctx.scale(-1, 1);
            drawMoose(-210, 132);
        }
    }
